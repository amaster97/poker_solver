...
class SolveRunner:
    ...
    def start(
        ...
        *,
        ...
        # v1.11: no pre-solve tree-size guard. Flop solves are tractable on
        # main's fast engine (rayon + suit-iso + inclusion-exclusion terminal
        # eval), and the marquee postflop path the user actually hits forces
        # ``rvr_mode=True`` → the vector-form RvR aggregator, which manages its
        # own subgame. The GUI-branch ``SolveTooLargeError`` refusal was built
        # against the old engine and is removed here.
    ...
    def _dispatch_solve(
        ...
        *,
        ...
        # 2. HUNL postflop — route through the canonical FAST hookless
        # binding on main's v1.11 engine (Integration §2, Option A).
        #
        ...
        # ``start()``'s default of "rust"). The fast Rust postflop path on
        # main (``solver.solve`` → ``_solve_rust`` → ``_rust.solve_hunl_postflop``
        # + ``_compute_exploitability_rust``) does NOT expose the mid-solve
        # ``on_progress`` / ``should_stop`` hooks the GUI branch built against
        # an older engine, so we deliberately do NOT thread them here. The UX
        # degrades cleanly UI-side: progress is shown as an indeterminate
        # "solving…" spinner (the worker sets status=running + target_iterations
        # and the post-return block appends the final exploitability point),
        # and Stop is a coarse thread-abandon (``_stop_event`` set, status
        # "stopped", the daemon's eventual result discarded). This branch is
        # reached ONLY when the dev-gate is ON (production forces rvr_mode=True,
        # which routes through ``_run_true_nash_rvr_path`` instead). The Rust
        # tier is used unconditionally; the Python reference engine is only the
        # silent in-extension fallback inside ``_solve_rust`` when the binding
        # is genuinely unavailable — never a user-facing engine choice.
