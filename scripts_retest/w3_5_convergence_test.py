"""W3.5 convergence test — does the 15-class PoC range converge to AA pure check at 3000 iter?

Hypothesis A vs B test:
- If AA check -> 1.0000 at 3000 iter: convergence issue (Hypothesis A)
- If AA check stays low: class-expansion wrapper bug (Hypothesis B)
"""
import time, json
from poker_solver import Card, HUNLConfig, Street, solve_range_vs_range_nash

cfg = HUNLConfig(
    starting_stack=10000, small_blind=50, big_blind=100, ante=0,
    starting_street=Street.RIVER,
    initial_board=tuple(Card.from_str(c) for c in ('Ts','8s','6s','4c','2d')),
    initial_pot=200, initial_contributions=(100,100),
    initial_hole_cards=(), postflop_raise_cap=2,
    bet_size_fractions=(0.33,0.75,1.50), include_all_in=False,
)
classes = ['AA','KK','QQ','JJ','TT','99','88',
           'AKs','AQs','AJs','KQs','KJs','JTs','98s','87s']

t0 = time.time()
result = solve_range_vs_range_nash(cfg, hero_range=classes, villain_range=classes,
    iterations=3000, hero_player=1, compute_exploitability_at_end=True)
wall = time.time() - t0
aa = result.per_class_strategy.get('AA', {})
print(f'15-class @ 3000 iter: AA check={aa.get("check",0):.4f}, wall={wall:.1f}s')
print(f'AA strategy: {dict(aa)}')
print(f'Range aggregate check: {result.range_aggregate.get("check",0):.4f}')
print(f'Exploitability: {result.exploitability:.4f}')

out = {
    "wall_s": wall,
    "n_classes": len(classes),
    "iterations": 3000,
    "aa_check": aa.get('check', 0),
    "aa_strategy": dict(aa),
    "range_aggregate_check": result.range_aggregate.get('check', 0),
    "exploitability": result.exploitability,
    "hypothesis_a_convergence": aa.get('check', 0) >= 0.90,
    "hypothesis_b_wrapper_bug": aa.get('check', 0) < 0.90,
    "verdict": "convergence_issue" if aa.get('check', 0) >= 0.90 else "wrapper_bug",
}
with open('/tmp/persona_retests/w3_5_convergence_3000iter_result.json', 'w') as fh:
    json.dump(out, fh, indent=2, default=str)
print(f'Wrote /tmp/persona_retests/w3_5_convergence_3000iter_result.json')
print(f'Verdict: {out["verdict"]}')
