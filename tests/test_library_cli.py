...
def test_cli_library_stats(tmp_path: Path) -> None:
    ...
    assert "1" in populated_proc.stdout, (
...
def test_cli_batch_solve_dry_run(tmp_path: Path) -> None:
    ...
    ), f"batch-solve --dry-run output missing dry-run indicator; got: {stdout!r}"
