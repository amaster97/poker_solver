# Proposed README.md update — 2026-05-23

## STATUS: SUPERSEDED

This draft was written on 2026-05-23 for a proposed README rewrite
during the v1.5.1 → v1.6.0 trajectory. Since then:

- v1.6.0, v1.7.0, and v1.7.2 have all shipped to public origin/main.
- The v1.4.0 `.dmg` `nicegui`-missing diagnosis described below was
  superseded first by PR 44 (which repackaged the `.dmg` for v1.6.0)
  and then by PR #42 (2026-05-26), which patched a **critical
  fork-bomb on Finder launch** in the v1.6.0 `.dmg`. The v1.6.0
  `.dmg` asset has been retroactively removed from the GitHub release.
- The current canonical user-facing docs are:
  - [`README.md`](../README.md) — top-level project overview.
  - [`docs/dmg_spawn_loop_rca_2026-05-26.md`](dmg_spawn_loop_rca_2026-05-26.md)
    — root-cause analysis for the v1.6.0 `.dmg` fork-bomb.

The body below is retained as **archival reference only**. Do NOT
apply it to `README.md`.

---

**Status (original):** DRAFT for user review. Do NOT apply directly to `README.md`
until reviewed.

**Why this draft exists.** The production `README.md` is stale at v1.0.0
and hasn't tracked the v1.0 → v1.5.1 trajectory. The draft also reflects
two empirical verdicts that landed today:

1. The v1.4.0 `.dmg` binary fails to launch on a clean Mac
   (`nicegui` missing from the PyInstaller bundle, plus Gatekeeper /
   arch / version-stamp issues — full report at
   `docs/dmg_v1_4_0_smoke_verification.md`). The `.dmg` is therefore
   demoted to "experimental" and the source install is the documented
   primary path.
2. The v1.5.0 acceptance test fails today. Three code reviews confirm
   the Rust vector-form CFR is structurally faithful to Brown's
   `trainer.cpp:138-240`, but a v1.6.1-bundle dry-run empirically
   re-confirmed a 33-pp divergence on bottom-pair-Ace cells at
   deep-cap facing-raise (A83, `b1000r3000`) — a label-vs-semantics
   gap (structure matches, output diverges). v1.6.1 ship HELD pending
   investigation. Shallow-cap, river single-shot, push/fold preflop
   workflows unaffected.

---

## Proposed new README.md content
