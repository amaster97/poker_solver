> **ARCHIVED — imported 2026-05-27 from unmerged branch `pr-93-terminal-utility-ablation` (commit `986f48d`).** This ablation was a LOCAL investigation that was never opened as a GitHub PR. Its empirical finding — that the rust convention introduced a 12-50pp regret-update bias vs. the canonical Brown convention at deep cap — motivated and is SUPERSEDED by **PR #78 (`37e5be1`, merged on `main`)**, which deleted the rust convention path and shipped a single canonical `utility()`. Retained here only as the audit-trail artifact for the v1.8.0 release notes' "PR #93 ablation" / "12-50pp gap" claims. Filename suffix `_archived` distinguishes this from a load-bearing current doc. See `feedback_brown_convention_adopt.md` for the canonical convention.
