...
the detailed record of what happened. The GUI-audit work is MERGED and shipped at `a9e92ea`
(the integration landed via `b551b13` / `f11f19f` / `3d469e4`).

**Legend:** `[x]` done · `[~]` in progress · `[ ]` queued · `[E]` engine-gated · `[?]` needs user decision

## ✅ VERIFICATION STATUS (shipped at `a9e92ea`)
- **Test-verified:** full UI suite green. The former 2 pre-existing pr24b "failures" are gone — the rejected `SolveTooLargeError` guard was REMOVED and those tests (`test_locked_strategies_threads_through_solve_runner_start`, `test_force_tree_solve_flag_threads_through_runner`) were rewritten to exercise the `force_tree_solve` bypass and now PASS. Auto-range 11 / chain-solve 13 / etc. pass. ruff clean.
- **Preview-browser verified (render + awaited-inline):** light mode, no-Python, hamburger, library, toggles, header/blinds, U13 message, F07 bar, U04 route badge, U06 deeper lines, the marquee solve (tree + mixed strat), P1 board picker, P5 reset, G1/G3 preset ranges, N5 chart label, clean boot of card-graphics + auto-range control + chain-solve walkthrough panel.
- **Deferred-refresh / tab-switch behaviors** (live preset/reset repaint, auto-range Apply live repaint, chain-solve walkthrough interactions, CHAIN-SOLVE tab layout) — covered by the integration tests and shipped; the `ui.timer` repaint path is in production.
...
## RESOLVED — ENGINE INTEGRATION (was DEFERRED/engine-gated; now SHIPPED)
- **⚙️ ENGINE INTEGRATION LANDED:** all v1.11 engine work (rayon → suit-iso+IE → preflop restricted-EV → bet-size menu: per-street menus + lean raise multipliers + flop-no-donk) was reconciled with the GUI's progress/cancel hooks and SHIPPED. The GUI audit UI now rides on the fast engine in one build (`b551b13` reconcile + `f11f19f` test-contract alignment + `3d469e4` merge → shipped at `a9e92ea`). RvR flops are tractable on the fast engine; the bet-size menu is concrete; the fast binding has the UI progress/cancel hooks.
- [E] Rich strategy/action display — raise + multiple bet sizes + mixed frequencies (rides the RvR engine).
- [E] Bet-size menu UI + street/tree structure — the engine defines these; UI reads bet sizes from config so it absorbs the menu.
- [x] **Engine↔GUI integration / MERGE** — DONE. Main's fast IE/vector/suit-iso engine reconciled with the UI progress/cancel hooks (`b551b13` / `f11f19f` / `3d469e4`); shipped at `a9e92ea` (public origin + backup mirror).
- [x] **`SolveTooLargeError` guard REMOVED.** The user-rejected "tree too large" patchup ([[feedback_no_patchups]]) is gone. The two pr24b tests it used to pre-empt (`test_locked_strategies_threads_through_solve_runner_start`, `test_force_tree_solve_flag_threads_through_runner`) were rewritten to exercise the `force_tree_solve` bypass against the fast engine and now PASS.
